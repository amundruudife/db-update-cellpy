...
def filter_by_projects(copied_file_path, sheet_name, projects, project_col_index=2):
    ...
    try:
        if not Path(copied_file_path).exists():
            raise FileNotFoundError(f"Copied file not found: {copied_file_path}")
        
        # Read the Excel file - skip the first 4 header rows for cellpy format
        df = pd.read_excel(copied_file_path, sheet_name=sheet_name, skiprows=4)
        ...
        logger.error(f"Error filtering by projects: {e}")
        raise
...
def check_duplicates(filtered_df, db_path, target_sheet, unique_key_col, key_col_index=0):
    ...
    try:
        ...
        # Skip header rows and read from row 5 onwards (index 4)
        existing_df = pd.read_excel(db_path, sheet_name=target_sheet, header=None, skiprows=4)
